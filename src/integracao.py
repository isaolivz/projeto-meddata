...
def gerar_dim_municipio(df_ibge):
    ...
    dim['estado'] = dim['estado'].fillna('Não informado')
...
def gerar_dim_hospital(df_cnes, df_municipios):
    ...
    if 'id_hospital' not in df_cnes.columns:
        logger.error("❌ df_cnes não tem id_hospital!")
        logger.info(f"Colunas disponíveis: {df_cnes.columns.tolist()}")
    ...
    dim['estado_hospital'] = dim['estado_hospital'].fillna('Não informado')
    ...
    if 'id_hospital' not in dim.columns:
        ...
        logger.info(f"Colunas disponíveis: {dim.columns.tolist()}")
...
def gerar_dim_tempo(df_sih):
    ...
    if 'data_internacao' not in df_sih.columns:
        logger.warning("Coluna 'data_internacao' não encontrada.")
    ...
    logger.info(f"Datas após filtro 2024: {len(dim):,}")
    ...
    logger.info(f"DIM_TEMPO gerada: {len(dim):,} datas únicas (apenas 2024)")
    return dim
...
def gerar_fato_internacao(df_sih, df_hospitais, df_municipios, df_tempo):
    ...
    if 'id_hospital' not in df_hospitais.columns:
        logger.error("❌ df_hospitais não tem id_hospital!")
        logger.info(f"Colunas disponíveis: {df_hospitais.columns.tolist()}")
    ...
    if len(df_tempo) > 0:
        ...
        logger.warning("DIM_TEMPO vazia - não será possível adicionar tempo_id")
    ...
    fato['estado_paciente'] = fato['estado_paciente'].fillna('Não informado')
